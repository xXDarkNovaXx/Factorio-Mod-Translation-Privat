...
local function repair_area(warden)
  local range = warden.variants[warden.name].search_radius
  local area = {
      {warden.entity.position.x - range, warden.entity.position.y - range},
      {warden.entity.position.x + range, warden.entity.position.y + range}
  ...
  if best_repair_target and best_repair_target.valid then
    ...
    local to_repair = math.min(best_repair_target.prototype.max_health - best_repair_target.health, active_search_interval * best_repair_target.prototype.repair_speed_modifier)
    best_repair_target.health = best_repair_target.health + to_repair
    warden.entity.get_inventory(defines.inventory.car_trunk).find_item_stack("repair-pack").drain_durability(to_repair / best_repair_target.prototype.repair_speed_modifier)
        ...
        if warden.entity.valid then
          if (warden.mode == "active" and (game.tick + warden.unit_number) % active_search_interval == 0) or
            ...
            if warden.entity.burner.remaining_burning_fuel > 0 then
                local can_repair = warden.entity.get_inventory(defines.inventory.car_trunk).get_item_count("repair-pack") > 0
...
script.on_event(defines.events.on_preplayer_mined_item, function(event) if wardens[event.entity.unit_number] then wardens[event.entity.unit_number]:die() end end)
