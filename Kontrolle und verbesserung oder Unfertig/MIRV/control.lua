script.on_event(defines.events.on_put_item, function(event)
  ...
  local target_smoke = surface.create_entity{name = "mirv-smoke", position = position}
  surface.create_entity{name = "mirv-smoke-2", position = position}
  player.force.chart(surface,{{position.x-80, position.y-80},{position.x+80, position.y+80}})
  -- local target = player.surface.create_entity{name = "mirv-target", position = position}
  -- player.surface.create_entity{name = "atomic-rocket", position = {position.x, position.y-1000}, target = target, speed = rand(-1,1)}
  ...
  for k, player in pairs (game.connected_players) do
    surface.create_entity{name = "mirv-sound", position = player.position}
    player.add_custom_alert(target_smoke, {type = "item", name = "mirv-item"}, {"launch-detected"}, true)
  end
end)
